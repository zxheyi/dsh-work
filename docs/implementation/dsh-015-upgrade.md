...
### Final package and acceptance-script checks

The refreshed unsigned macOS arm64 package is bound to product commit `faf84562107bff7b9d1b110f842d3a7655e599ec`. `pnpm package:desktop`, `pnpm notices:verify-release` (503 packages, zero blockers), and `pnpm test:package` passed. Bundle SHA-256 is `f1935c72f384322ca74f1200288038c73ae19902eb377b15046049595ba5abf2`; the relocated application passed two launches and clean shutdown while ignoring the developer Node override.

The subsequent acceptance-script-only change updates the remaining native upload selectors and reads canonical V3 assistant messages in live-evidence assertions. `node --test tests/live-release-evidence.test.ts scripts/verify-contract.test.mjs` passed 17 checks. `DSH_WORK_NODE=<verified Node> node scripts/run-runtime-context-test.mjs` passed: original Session, draft, history and output selection survived runtime stop, an injected failed launch, and explicit retry without replay or a new conversation. The live-provider suite itself was not run. These test/documentation changes do not change the packaged product bytes.
