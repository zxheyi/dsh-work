...
- A successful copy appends the native `@path` grammar to that Session's existing draft. Pending resources and drafts remain isolated while switching between Sessions, return with their owning Session, and clear after the native request consumes the draft. Copied paths are promoted through the native `slash/input-insert-reference` event and `reference` codec into inline file-name chips. The native editor owns their cursor, deletion, undo and send behavior; copy completion does not claim the model read the file. Copying and failed items alone occupy a compact, non-overlapping input dock.
...
### T22 Four-design product acceptance

- Froze fifteen real Electron screenshots: one actual state for each of the four design images plus model-unavailable, generation, empty Workspace, empty search, permission-denied, revision-failure, Runtime-restored and content/version panels at 736px and 390px. The comparison preserves the native DSH surface and records intended composition differences instead of claiming pixel identity with concept art. Version-panel geometry now proves all four actions stay contained at each width.
- Added a machine-readable evidence manifest and verifier. It requires all four design mappings, all eight PRD scenarios, all F1-F13 rows and every supplemental state; rejects unknown criterion or command references; requires F1-F12 coverage and frozen passing result JSON for the exact command set; verifies the pre-acceptance product base is an ancestor and pins the exact renderer/E2E sources changed during acceptance; fully parses chunk checksums and decodes bounded PNG scanlines; and pins every result and actual screenshot by SHA-256.
- F1-F12 have automated macOS evidence. T22 remains pending because its T14 dependency still lacks the required five-person F13 observation. Live production-model and Windows claims also remain explicitly absent. No automated run or fixture is counted as a human result.
- Verification: `pnpm verify:familiar-v5`, `pnpm check`, `pnpm test`; real Electron `test:conversation`, `test:workspace-sessions`, `test:session-navigation`, `test:session-resource`, `test:session-permission`, `test:session-output` and `test:runtime-context`; Runtime/Loader integration and desktop lifecycle gate. The first conversation rerun exposed and fixed an overly broad body-text equality assertion: the stable check now proves Enter creates no Turn and preserves the draft while allowing the upstream model dialog to render.
