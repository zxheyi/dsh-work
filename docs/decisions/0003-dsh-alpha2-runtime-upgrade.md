...
This record supersedes ADR 0002 after native evidence review and the user's instruction to proceed. The old runtime and its historical Windows evidence remain unchanged. Research [source provenance](../../prototypes/m0-runtime-upgrade/source-pin.json) and [runtime artifacts](../../prototypes/m0-runtime-upgrade/runtime-pin.json) retain their original candidate labels so the tested input hashes remain reproducible. The active machine-readable selection is [runtime/baseline.json](../../runtime/baseline.json); acceptance is not an implementation-complete or signed distribution manifest.
