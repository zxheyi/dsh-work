...
Diagnostic commit `f0d1dd4` then passed locally and on native macOS, but [CI run 33461836305](https://github.com/zxheyi/dsh-work/actions/runs/33461836305) reproduced the Windows failure. Provenance passed before verification and after unit, contract and runtime checks; it failed after the four Electron modes with fixed code `package-provenance-failed`. This rules out those earlier phases and the source, Node, family and lock sub-checks for that attempt. It does not yet identify one Electron mode or distinguish changed package bytes from a Windows read/inventory failure. The next diagnostic split runs each existing mode separately and applies the unchanged package gate immediately after each mode.

Diagnostic commit `d601418` passed locally and on native macOS. [CI run 33462277945](https://github.com/zxheyi/dsh-work/actions/runs/33462277945) showed Windows provenance passing after normal, missing-runtime and renderer-crash modes, then failing with `package-provenance-failed` immediately after runtime-recovery. All behavior checks in that mode passed. The remaining package diagnostic keeps the same integrity and byte comparisons but assigns fixed codes to archive/integrity, installed metadata, inventory and installed bytes. Raw filesystem errors remain hidden. This is still diagnosis, not a retry policy or a claim of process-tree cleanup.

Diagnostic commit `d17ab31` passed locally and on native macOS. [CI run 33462737731](https://github.com/zxheyi/dsh-work/actions/runs/33462737731) reproduced `package-metadata-failed` at the same Windows checkpoint. The final diagnostic distinction separates an unreadable/unparseable installed manifest (`package-metadata-unavailable`) from a successfully read manifest whose accepted name, version or launcher fields differ (`package-metadata-mismatch`). Neither outcome is retried or accepted as clean.

Diagnostic commit `fd16b5a` passed locally and on native macOS. [CI run 33463087572](https://github.com/zxheyi/dsh-work/actions/runs/33463087572) reproduced `package-metadata-unavailable` immediately after the Windows runtime-recovery mode. The installed root package still resolved, so this is not a missing pnpm link and not a readable manifest with changed accepted fields. The recovery E2E now retains the original official manifest bytes and compares them after the recovered child stops, after deleting only its owned temporary Home, and after destroying the status window. These are assertions, not retries: any read failure or byte change fails the mode with a fixed phase before the outer provenance gate runs.

Diagnostic commit `a11bebb` passed on native macOS. [CI run 33463742284](https://github.com/zxheyi/dsh-work/actions/runs/33463742284) proved the Windows manifest matched byte-for-byte after the recovered child stopped, then became unavailable immediately after recursively deleting the test-owned Home. That Home contains a directory junction from its Profile to the repository's pnpm-managed `@deepseek-ai/dsh-cmdline` package. The failure is therefore test cleanup escaping through a Windows junction, not Harness behavior or an incomplete process shutdown. The repair candidate validates and unlinks only that test-created junction before recursively deleting the owned Home; the immutable manifest assertions and all provenance gates remain unchanged, with no retry or delay.

The first repair candidate `841b205` passed locally and on native macOS, but [CI run 33464295315](https://github.com/zxheyi/dsh-work/actions/runs/33464295315) remained red at the same Windows assertion. Removing only the initial `dsh-cmdline` junction was insufficient because normal Harness profile boot also materializes a shared installation dependency fallback beneath the same owned Home. The refined cleanup is test infrastructure only: it accepts only a direct `dsh-work-*` child of the OS temporary directory, refuses a linked root, traverses directories with `lstat` without following links, unlinks every nested symlink/junction, and only then removes the owned tree. A focused unit test proves an external link target survives. Harness source and lifecycle behavior remain untouched.

Repair commit `50bb158` passed [CI run 33464734313](https://github.com/zxheyi/dsh-work/actions/runs/33464734313) on Windows x64 and macOS arm64. Both clean jobs completed every unit/contract check, six official CLI cases, four Electron modes and every staged provenance comparison. Runtime recovery observed `exit` then `close` for both generations, required explicit renderer retry, and preserved the official runtime manifest after recovered stop, owned-Home cleanup and window destruction. The accepted scope remains direct-child recovery while the Electron host survives; complete process-tree and main-host-death ownership are still a separate decision.
