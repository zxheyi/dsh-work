...
export default function lifecycle(ctx: LifecycleContext): void {
  const appReady = ctx.appReady
  if (!process.send || !appReady || !ctx.appExit) throw new Error('desktop lifecycle unavailable')
  ...
  ctx.effect(() => appReady.onReady(() => emit('ready')))
  ctx.effect(() => () => {
    emit('disposed')
    process.stdin.pause()
  })
