...
> **当前状态：** 已选定 Electron 与官方 Harness alpha.2，并进入最小生命周期桌面开发。暂时没有可供日常使用的版本；完整工作界面、原生跨平台和安装包验收尚未完成。本文中的产品功能仍为目标方向。
...
仓库已进入 Electron 生命周期开发预览，启动与验证命令见 [运行时开发说明](runtime/README.md)，验收边界见 [最小生命周期切片](docs/acceptance/electron-lifecycle-slice.md)。Phase 0 已建立仓库契约；开始工作前请阅读 [`AGENTS.md`](AGENTS.md) 与 [`docs/workflow.md`](docs/workflow.md)。AI Native 交付工作流的中文 v1 冻结记录见 [`docs/workflow-v1.zh-CN.md`](docs/workflow-v1.zh-CN.md)。当前产品范围、首个里程碑和架构决策流程分别见 [`docs/product-scope.md`](docs/product-scope.md)、[`docs/acceptance/m0.md`](docs/acceptance/m0.md) 和 [`docs/decisions/README.md`](docs/decisions/README.md)。
...
技术栈与上游版本已通过 ADR 接受；首个产品里程碑仍需完成原生 Windows 产品验证、进程树回收、完整工作界面和打包验收。
