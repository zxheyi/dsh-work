...
Run `pnpm notices:prepare-native` to download/cache the 407 hash-locked materials (about 216 MiB). `pnpm package:desktop` performs this preparation automatically and builds a separate `DSH-Work-<version>-<platform>-<arch>-sources.tar.gz` asset. License, attribution, build and replacement texts remain under the application's `third-party/native` resource directory; source archives move to the companion asset. No application-startup download is introduced. Verification works offline once materials are cached. Packaging fails on a failed download or digest mismatch instead of silently substituting current upstream contents.

`pnpm notices:verify-release` checks every native component's corresponding-source mapping, package versions, library bytes, source/notice/build/replacement material bytes, and a native addon replacement smoke. Unknown package versions/platforms, absent materials and changed binaries fail closed. This replaces the unconditional blocker tracked in [Issue #38](https://github.com/zxheyi/dsh-work/issues/38) with verifiable evidence. Under [ADR 0023](../docs/decisions/0023-compact-desktop-distribution.md), `pnpm package:archive` verifies the companion archive against its identity embedded in the app, the material lock and actual native binaries. It writes the application archive, matching source archive, common receipt and checksums to `artifacts/release/`; CI uploads them together. Offer both assets in the same download entry. Publishing a binary-only entry is not what this gate verifies.
