...
for (const phase of ['missing', 'connected']) {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), `dsh-work-conversation-${phase}-`))
  const reportPath = path.resolve(`artifacts/conversation/${phase}-result.json`)
  fs.mkdirSync(path.dirname(reportPath), { recursive: true })
  fs.writeFileSync(reportPath, JSON.stringify({ status: 'fail', phase, step: 'launch' }, null, 2))
  const child = spawn(electron, ['tests/native-conversation-e2e.ts'], {
    shell: false,
    env: {
      ...process.env,
      DSH_WORK_NODE: node,
  ...
  if (exit !== 0 || report?.status !== 'pass') {
    console.error(`Native conversation acceptance failed in ${phase}/${report?.step || 'launch'}`)
    process.exitCode = 1
    break
  }
}
...
if (process.exitCode === undefined) {
  console.log('Native conversation acceptance passed: missing connection, model selection, two-turn chat, and Stop')
}
