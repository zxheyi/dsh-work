...
const home = fs.mkdtempSync(path.join(os.tmpdir(), 'dsh-work-session-permission-'))
const reportPath = path.resolve('artifacts/session-permission/result.json')
fs.mkdirSync(path.dirname(reportPath), { recursive: true })
fs.writeFileSync(reportPath, JSON.stringify({ status: 'fail', step: 'launch' }, null, 2))
const child = spawn(electron, ['tests/session-permission-e2e.ts'], {
  shell: false,
...
if (exit !== 0 || report?.status !== 'pass') {
  console.error(`Session permission acceptance failed in ${report?.step || 'launch'}`)
  process.exitCode = 1
} else {
  console.log('Session permission acceptance passed: allow once, reject, restore, isolate, and continue')
}
