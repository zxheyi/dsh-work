...
const home = fs.mkdtempSync(path.join(os.tmpdir(), 'dsh-work-session-navigation-'))
const reportPath = path.resolve('artifacts/session-navigation/result.json')
fs.mkdirSync(path.dirname(reportPath), { recursive: true })
fs.writeFileSync(reportPath, JSON.stringify({ status: 'fail', step: 'launch' }, null, 2))
const child = spawn(electron, ['tests/session-navigation-e2e.ts'], {
  shell: false,
...
if (exit !== 0 || report?.status !== 'pass') {
  console.error(`Session navigation acceptance failed in ${report?.step || 'launch'}`)
  process.exitCode = 1
} else {
  console.log('Session navigation acceptance passed: switch, drafts, search, no-result exit, and archive')
}
