...
const home = fs.mkdtempSync(path.join(os.tmpdir(), 'dsh-work-session-resource-'))
const reportPath = path.resolve('artifacts/session-resource/result.json')
fs.mkdirSync(path.dirname(reportPath), { recursive: true })
fs.writeFileSync(reportPath, JSON.stringify({ status: 'fail', step: 'launch' }, null, 2))
const child = spawn(electron, ['tests/session-resource-e2e.ts'], {
  shell: false,
